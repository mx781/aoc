-- SETUP
SET @input_small = "b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10";

SET @input_large = "aj dec -520 if icd < 9
z dec -500 if b <= 2
zz dec 628 if z >= 499
db dec -818 if u >= 0
...
CREATE TABLE IF NOT EXISTS `registers` (
  ...
  `value` INT(11),
  `max_value` INT(11) DEFAULT -9999,
  PRIMARY KEY (`name`)
);
CREATE TABLE IF NOT EXISTS `instructions` (
  `line` VARCHAR(1024) NOT NULL,
  `mutate_var` VARCHAR(5),
  `operation` VARCHAR(3),
  `diff` BIGINT,
  `compare_var` VARCHAR(5),
  `compare_op` VARCHAR(5),
  `compare_value` INT(11)
);

REPLACE INTO `input` (`id`, `input`) VALUES ("small", @input_small);
REPLACE INTO `input` (`id`, `input`) VALUES ("large", @input_large);
...
SET @solve = "large";
SET @input = (SELECT input FROM input i WHERE i.id = @solve);

DROP PROCEDURE IF EXISTS parse_input;
DROP PROCEDURE IF EXISTS process_instructions;

DELIMITER $$

CREATE PROCEDURE parse_input() BEGIN
    DECLARE it INT;
    DECLARE result VARCHAR(1024);
    
    DECLARE mutate_var, compare_var, compare_op, compare_value VARCHAR(5);
    DECLARE operation, diff VARCHAR(64);
    
    SET it = 1;
    SET result = SPLIT_STRING(@input, "\n", it);
    parse: LOOP
        IF result = "" THEN
            LEAVE parse;
        ELSE
            SET mutate_var = SPLIT_STRING(result, ' ', 1);
            SET operation = SPLIT_STRING(result, " ", 2);
            SET diff = SPLIT_STRING(result, " ", 3);
            SET compare_var = SPLIT_STRING(result, " ", 5);
            SET compare_op = SPLIT_STRING(result, " ", 6);
            SET compare_value = SPLIT_STRING(result, " ", 7);
            ...
            INSERT INTO instructions
                (line, mutate_var, operation, diff, compare_var, compare_op, compare_value)
            VALUES
                (result, mutate_var, operation, CAST(diff AS SIGNED), compare_var, compare_op, CAST(compare_value AS SIGNED));
                
            -- store registers w/ initial 0 values
            REPLACE INTO registers (name, value) VALUES(mutate_var, 0);
...
CREATE PROCEDURE process_instructions() BEGIN
    
    DECLARE mutate_var_f, compare_var_f, compare_op_f, compare_value_f VARCHAR(5);
    DECLARE operation_f, diff_f VARCHAR(64);
    DECLARE res TINYINT(1);
--  DECLARE upd_query VARCHAR(1024);
    
    DECLARE done INT DEFAULT FALSE;
    
    DECLARE curs CURSOR FOR SELECT mutate_var, operation, diff, compare_var, compare_op, compare_value FROM instructions;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    
    OPEN curs;
    iterator: LOOP
        FETCH curs INTO mutate_var_f, operation_f, diff_f, compare_var_f, compare_op_f, compare_value_f;
        IF done THEN
            LEAVE iterator;
        END IF;
        SET @check_condition = CONCAT("SELECT @res := IF(value ", compare_op_f, " ", compare_value_f, ", TRUE, FALSE) FROM registers WHERE name = '", compare_var_f, "';");
        PREPARE check_statement FROM @check_condition;
        EXECUTE check_statement;
        DEALLOCATE PREPARE check_statement;
                
        IF @res = 1 THEN
            /* IF operation_f = "inc" THEN
                SET @upd_query = "UPDATE registers r SET r.value = r.value + ? WHERE r.name = ?;";
            ELSE
                SET @upd_query = "UPDATE registers r SET r.value = r.value - ? WHERE r.name = ?;";
            END IF; */
            
            IF operation_f = "inc" THEN
                SET @upd_query = "UPDATE registers r SET r.max_value = IF(r.value + ? > r.max_value, r.value + ?, r.max_value), r.value = r.value + ? WHERE r.name = ?;";
            ELSE
                SET @upd_query = "UPDATE registers r SET r.max_value = IF(r.value - ? > r.max_value, r.value - ?, r.max_value), r.value = r.value - ? WHERE r.name = ?;";
            END IF;
            
            SET @diff_in = CAST(diff_f AS SIGNED);
            SET @mutate_var_in = mutate_var_f;
            PREPARE upd_statement FROM @upd_query;
            EXECUTE upd_statement USING @diff_in, @diff_in, @diff_in, @mutate_var_in;
            DEALLOCATE PREPARE upd_statement;
        END IF;
    END LOOP iterator;
...
SELECT MAX(value) as part_1, MAX(max_value) as part_2 FROM registers;
